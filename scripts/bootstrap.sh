...
Anschliessend bootstrap.sh erneut starten." 18 70
    ...
    # ── Optionales Recovery-Site Passwort ──
    RECOVERY_PASSWORD=""
    if whiptail --backtitle "DNS Homelab Bootstrap" --title "Recovery-Site Auth" \
        --yesno "Recovery-Webseite (Dashboard + Operations) mit\nPasswort schuetzen?\n\nEmpfohlen — die Operations-Seite kann das System\nbeeinflussen (Container, Sync, Updates, Reboot)." 12 70 \
        --yes-button "Ja, Passwort setzen" --no-button "Nein, ohne Auth"; then
        while true; do
            local pw1 pw2
            pw1=$(whiptail --backtitle "DNS Homelab Bootstrap" --title "Recovery-Passwort" \
                --passwordbox "Passwort fuer Recovery-Webseite\n(min. 6 Zeichen):" 10 70 \
                3>&1 1>&2 2>&3) || die "Abgebrochen."
            pw2=$(whiptail --backtitle "DNS Homelab Bootstrap" --title "Recovery-Passwort" \
                --passwordbox "Passwort wiederholen:" 10 70 3>&1 1>&2 2>&3) || die "Abgebrochen."
            if [[ "$pw1" == "$pw2" && ${#pw1} -ge 6 ]]; then
                RECOVERY_PASSWORD="$pw1"
                break
            fi
            whiptail --backtitle "DNS Homelab Bootstrap" --title "Fehler" \
                --msgbox "Passwoerter stimmen nicht ueberein oder zu kurz." 8 60
        done
    fi

    # ── Bestaetigung ──
    local auth_label="ohne"
    [[ -n "$RECOVERY_PASSWORD" ]] && auth_label="mit Passwort"
...
"Mit dieser Konfiguration fortfahren?
  ...
  Interface:    $IFACE
  Repo:         $REPO_URL
  Recovery-UI:  $auth_label
" 18 70 || die "Abgebrochen."
...
RECOVERY_PASSWORD=$(printf '%q' "${RECOVERY_PASSWORD:-}")
...
apply_network() {
    ...
    # Strategie: Debian-native ifupdown verwenden. Standard auf Debian 13 minimal.
    # Wir aendern nur /etc/network/interfaces von DHCP auf static und deaktivieren
    # konkurrierende Dienste. Keine Purges, keine Service-Massenaktionen.

    # ── 1. systemd-resolved deaktivieren (Port 53 fuer Pi-hole frei) ──
    if systemctl is-enabled --quiet systemd-resolved 2>/dev/null \
       || systemctl is-active  --quiet systemd-resolved 2>/dev/null; then
        log "Deaktiviere systemd-resolved (Port 53 fuer Pi-hole)"
        systemctl disable --now systemd-resolved 2>/dev/null || true
    fi

    # ── 2. cloud-init disablen falls installiert (nicht purgen!) ──
    if [[ -d /etc/cloud ]]; then
        touch /etc/cloud/cloud-init.disabled 2>/dev/null || true
    fi

    # ── 3. Konkurrierende systemd-networkd-Configs entfernen ──
    rm -f /etc/systemd/network/*.network 2>/dev/null || true

    # ── 4. resolv.conf statisch + immutable ──
...
EOF
    ...
    # ── 5. /etc/network/interfaces auf static umstellen (Debian-native) ──
    if [[ -f /etc/network/interfaces ]]; then
        cp /etc/network/interfaces /etc/network/interfaces.bak.$(date +%s) 2>/dev/null || true
    fi
    cat > /etc/network/interfaces <<EOF
# Generated by dns-bootstrap.sh
...
EOF
    ...
    # ── 6. Konkurrierende interfaces.d-Dateien entfernen ──
    if [[ -d /etc/network/interfaces.d ]]; then
        rm -f /etc/network/interfaces.d/* 2>/dev/null || true
    fi

    log "Netzwerk-Konfiguration geschrieben (ifupdown / networking.service)"
}
...
[Service]
Type=oneshot
# Kurze Pause damit ifupdown garantiert die statische IP vergeben hat
...
step_install_packages() {
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \
        ...
        python3-fastapi python3-uvicorn python3-bcrypt python3-pydantic
}
...
step_setup_auth() {
    local env_file="$REPO_DIR/compose/.env"
    local snippet="$REPO_DIR/compose/auth.snippet"

    if [[ -n "${RECOVERY_PASSWORD:-}" ]]; then
        log "Generiere bcrypt-Hash fuer Recovery-Passwort..."
        local hash
        hash=$(python3 -c "import bcrypt; print(bcrypt.hashpw('$RECOVERY_PASSWORD'.encode(), bcrypt.gensalt(rounds=10)).decode())") \
            || die "bcrypt-Hash-Generierung fehlgeschlagen"

        # In .env schreiben/aktualisieren
        {
        ...
        # auth.snippet schreiben
        cat > "$snippet" <<EOF
# auto-generated by bootstrap.sh — set-password aendert dies
basic_auth /* {
	admin $hash
}
EOF
        log "Recovery-Site Auth aktiviert"
    else
        # Auth deaktivieren (leer, aber Datei muss existieren wegen import)
        cat > "$snippet" <<'EOF'
# kein Passwort gesetzt — alle Pfade ohne Auth zugaenglich
EOF
        log "Recovery-Site Auth deaktiviert"
    fi
}

step_install_api() {
    log "Installiere homelab-api systemd-Service..."

    # Service-Datei
    install -m 0644 "$REPO_DIR/api/homelab-api.service" \
        /etc/systemd/system/homelab-api.service

    systemctl daemon-reload
    systemctl enable homelab-api.service
    systemctl restart homelab-api.service

    # Kurzer Healthcheck
    sleep 2
    if curl -sS --max-time 3 http://127.0.0.1:8088/api/health >/dev/null; then
        log "homelab-api laeuft (Port 8088)"
    else
        log "WARNUNG: homelab-api antwortet nicht — journalctl -u homelab-api pruefen"
    fi
}
